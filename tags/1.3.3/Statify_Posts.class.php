...
class Statify_Posts {
	...
	*/
	public static function get_posts($post_type, $post_category, $amount, $interval) {
		...
		foreach ($targets as $entry) {
			...
			// Frontpage label, if post_type page / postpage and "show on front" is not set to page (then frontpage is static page)
			if ($clear_url == '/' && ($post_type == 'page' || $post_type == 'postpage') && 'page' != get_option('show_on_front')) {
				...
				if (!isset($posts[0]['visits'])) $posts[0]['visits'] = 0;
	...
	*/
	public static function get_post_list($post_type, $post_category, $amount, $days) {
		return new WP_Query(array('post__in' =>array_keys(self::get_posts($post_type, $post_category, $amount, $days))));
	}
