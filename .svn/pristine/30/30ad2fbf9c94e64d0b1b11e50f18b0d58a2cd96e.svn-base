...
Die Gesamtaufrufe der Seite lassen sich über:
...
Dafür einfach die CSS Klassen:
* `.statify-widget-list { ... }`
* `.statify-widget-element { ... }`
* `.statify-widget-link { ... }`

= Liste von Numerisch auf Punkte ändern =

Wer die Aufzählung der Inhalte (1., 2., 3., ...) auf Aufzählungszeichen ändern will, muss dafür nur folgendes zum CSS hinzufügen:
`.statify-widget-element {
...
* Korrektur von einem Link Bug im Widget
* Shortcode für Gesamtanzahl Aufrufe

= 1.1.9 =

* Korrektur von einem Übersetzungsfehler.
* Listentyp Einstellung wurde doch wieder entfernt.

= 1.1.8 =

* Das Plugin ist nun für die Übersetzung vorbereitet.
* Man kann nun zwischen den Listentyp (`<ol>` oder `<ul>`) pro Widget wählen.
* Es sind nun die Klassen `statify-widget-list`, `statify-widget-element` und `statify-widget-link` als Klasse hinzugekommen um ein flexibleres Design machen zu können.

= 1.1.7 =

* Listentyp geordnet (`<ol>`) auf ungeordnet (`<ul>`) geändert, um höhere Theme Kompatibilität zu erreichen.
* Veraltete create_function() Methode ausgetauscht.
* Fehler bei Shotcode Verwendung entfernt (statify_count in Statify_Posts.class.php bearbeitet)
